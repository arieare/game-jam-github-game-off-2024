...
@export var movement_state_machine:class_state_machine
@export var position_state_machine:class_state_machine
@export var skill_state_machine:class_state_machine
...
func _physics_process(delta: float) -> void:
	#ray.levitate.levitate_actor(delta,self)
...
func look_at_cursor():
	global.look_at_target.look_at_target(look_aim, body_head, 0.2)
	global.look_at_target.look_at_target(look_aim, body, 0.03)	
...
func maintain_upright(delta):
	if player_input.get_direction() == Vector3.ZERO or self.angular_velocity.x > 15 or self.angular_velocity.x < -15 or self.angular_velocity.y > 5 or self.angular_velocity.y < -5 or self.angular_velocity.z > 15 or self.angular_velocity.z < -15:	
		...
		self.rotation.x = root.pid_controller.update_angle(self.rotation.x, target_upright, delta)
		self.rotation.z = root.pid_controller.update_angle(self.rotation.z, target_upright, delta)
		self.rotation.y = root.pid_controller.update_angle(self.rotation.z, target_upright, delta)
		
		self.angular_velocity.y = root.pid_controller.update(self.angular_velocity.y, target_upright, delta)
		self.angular_velocity.x = root.pid_controller.update(self.angular_velocity.x, target_upright, delta)
		self.angular_velocity.z = root.pid_controller.update(self.angular_velocity.z, target_upright, delta)
