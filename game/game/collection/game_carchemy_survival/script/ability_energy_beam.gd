...
@export var speed: float = 240.0 *60
@export var speed_fall: float = 10.0
...
func _bullet_process(delta:float)->void:
	...
	while remainLength > 0.001 && bounces > 0:
		...
		var query = PhysicsRayQueryParameters2D.create(pos, end)	
		data = spaceState.intersect_ray(query)
		if data:
