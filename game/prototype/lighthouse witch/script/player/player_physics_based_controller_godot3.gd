...
@onready var levRay:RayCast3D = $RayCast3D
...
func ground_check()->Dictionary:
	...
	if levRay.is_colliding():
		...
		collisionDistance = self.transform.origin.distance_to(collisionPosition)
...
func levitation(delta)->void:
	...
	if offset < 0:
		...
		if ground_check().collisionBody is RigidBody3D:
			...
			body.apply_force(contactPosition -  body.position, finalForce)
...
func movement(delta):
	...
	if self.linear_velocity.length() > 1:
		...
		pos = sin(time*5*PI)
		pos = clamp(pos,-0.12,0.12)
		self.apply_central_force(Vector3(0,pos,0))
	else:
		pos = sin(time*1.3*PI)
		pos = clamp(pos,-0.15,0.15)
		self.apply_central_force(Vector3(0,pos,0))		
		#$body.position.y = $body.position.y - pos
	

	
	apply_torque_impulse(-self.angular_velocity)
