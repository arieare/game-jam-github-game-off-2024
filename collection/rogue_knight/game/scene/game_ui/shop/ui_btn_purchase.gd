...
func _ready() -> void:
	util.root.data_instance.stylesheet.button_styler(self, price_text, "green", "medium", false, false)
	
	price_text.text = "¢" + item_root.patch_price_text
	
	if item_root.item_data["price"] > util.root.data_instance.game_data.money:
		util.root.data_instance.stylesheet.button_styler(self, price_text, "green", "medium", true, false)
	else:
		util.root.data_instance.stylesheet.button_styler(self, price_text, "green", "medium", false, false)	
