...
func look_at_cursor():
	util.look_at_target.look_at_target(look_aim, body_head, 0.2)
	util.look_at_target.look_at_target(look_aim, body, 0.03)	
...
func maintain_upright(delta):
	if player_input.get_direction() == Vector3.ZERO or self.angular_velocity.x > 15 or self.angular_velocity.x < -15 or self.angular_velocity.y > 5 or self.angular_velocity.y < -5 or self.angular_velocity.z > 15 or self.angular_velocity.z < -15:	
		...
		self.rotation.x = util.pid.update_angle(self.rotation.x, target_upright, delta)
		self.rotation.z = util.pid.update_angle(self.rotation.z, target_upright, delta)
		self.rotation.y = util.pid.update_angle(self.rotation.z, target_upright, delta)
		
		self.angular_velocity.y = util.pid.update(self.angular_velocity.y, target_upright, delta)
		self.angular_velocity.x = util.pid.update(self.angular_velocity.x, target_upright, delta)
		self.angular_velocity.z = util.pid.update(self.angular_velocity.z, target_upright, delta)
